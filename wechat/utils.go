...
//MessageRespond is rececived message respond
...
func (w *WorkWeixin) SendText(toUser string, toparties string, totag string, text string) (string, error) {
	...
	var resp MessageRespond
	json.Unmarshal(buffer, &resp)
	fmt.Println("resp", resp)

	if resp.InvalidUser != "" {
		log.Println("Invaliduser", resp.InvalidUser)
		return fmt.Sprintln("Invalid user: ", resp.InvalidUser), errors.New("invaliduser")
	}
	...
	if err != nil {
		...
		log.Println("string(buffer)", string(buffer))
...
func (w *WorkWeixin) getStoreFile() string {
	...
	return fmt.Sprintf(path+"/work_%d.json", w.agentId)
}
